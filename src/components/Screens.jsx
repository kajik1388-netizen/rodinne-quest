  ...
  return (
    <div style={{ padding:"16px 16px 0" }}>
      ...
      <div style={{ background:`linear-gradient(135deg,${DARK},#2C2C54)`, borderRadius:24, padding:"20px 12px 16px", marginBottom:12 }}>
        <p style={{ color:"rgba(255,255,255,0.4)", fontSize:11, fontWeight:800, textAlign:"center", margin:"0 0 16px", letterSpacing:1 }}>
          {view==="week" ? "TENTO TÝŽDEŇ" : "CELKOVO"}
        </p>
            ...
            return (
              <div key={m.id} style={{ display:"flex", flexDirection:"column", alignItems:"center", flex: isF?1.2:1 }}>
                ...
                <div style={{ background: isF?`${m.color}33`:"rgba(255,255,255,0.08)", border:`2px solid ${isF?m.color+"66":"rgba(255,255,255,0.1)"}`, borderRadius:14, padding:"8px 6px", marginTop:6, width:"100%", textAlign:"center" }}>
                  ...
                  <p style={{ color:YELLOW, fontSize:isF?17:13, fontWeight:900, margin:0 }}>
                    {view==="week" ? (m.weekPts||0) : (m.totalPts||0)}b
                  </p>
      ...
      {sorted[3] && (() => {
        const m = sorted[3]; const Av = AVTS[m.id];
        return (
          <Card style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12, border: m.id===member.id ? `2px solid ${m.color}` : "2px solid transparent" }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <p style={{ fontSize:11, color:"#aaa", margin:"2px 0 0" }}>🔥 {m.streak} dní</p>
            </div>
      ...
      <div style={{ background:`${member.color}12`, border:`1.5px solid ${member.color}44`, borderRadius:16, padding:"12px 16px", textAlign:"center" }}>
        <p style={{ color:member.color, fontSize:14, fontWeight:800, margin:0 }}>
          {motivations[member.id] || "Skvelá práca! 💪"}
        </p>
      </div>
    </div>
  );
                ...
                return (
                  <div key={r.id} style={{ background:"white", borderRadius:18, padding:"14px 16px", display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 8px rgba(0,0,0,0.05)", opacity:ok?1:0.6 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ fontSize:14, fontWeight:800, color:"#1A1A2E", margin:"0 0 2px", wordBreak:"break-word" }}>{r.name}</p>
            ...
            : <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {pending.map((r,i)=>(
                  <Card key={i} style={{display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}><p style={{fontSize:14,fontWeight:800,color:"#1A1A2E",margin:"0 0 2px",wordBreak:"break-word"}}>{r.name}</p><p style={{fontSize:11,color:"#aaa",margin:0}}>{r.date}</p></div>
                    <span style={{background:"#FFF3CD",color:"#F57F17",borderRadius:10,padding:"4px 10px",fontSize:11,fontWeight:800}}>⏳</span>
                  </Card>
                ))}
              </div>
        )}
...
export function Chat({ member, chat, setChat, privateChat, setPrivateChat, members, activeTasks, setActiveTasks, showToast }) {
  ...
  const [tradeForm, setTradeForm] = useState({
    task:"", offer:"body", offerAmt:5,
    to: member.id==="bart" ? "lisa" : "bart"
  });
  ...
  const sendTrade = () => {
    ...
    setChat(p=>[...p,{
      ...
      text:`🤝 ${member.name} žiada ${toM?.name}: Splníš za mňa "${tradeForm.task}"? Ponúkam: ${tradeForm.offer==="body"?`${tradeForm.offerAmt} bodov 💰`:tradeForm.offer==="item"?"predmet z inventára 🎁":"zadarmo 😊"}`,
      time:new Date().toLocaleTimeString("sk",{hour:"2-digit",minute:"2-digit"}),
      color:"#FF9800", unread:true,
      trade:{ from:member.id, to:tradeForm.to, task:tradeForm.task, offer:tradeForm.offer, offerAmt:tradeForm.offerAmt, status:"pending", id:`tr_${Date.now()}` }
    }]);
    setShowTrade(false);
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 82px)" }}>
      ...
      {showTrade && (
        <div style={{ background:"#FFF8E1", borderBottom:"1.5px solid #FFE082", padding:"12px 16px" }}>
          ...
          <input value={tradeForm.task} onChange={e=>setTradeForm(p=>({...p,task:e.target.value}))} placeholder="Názov úlohy..." style={{ ...iS, marginBottom:8 }}/>
          ...
          {tradeForm.offer==="body" && <input type="number" value={tradeForm.offerAmt} onChange={e=>setTradeForm(p=>({...p,offerAmt:Number(e.target.value)}))} placeholder="Počet bodov..." style={{ ...iS, marginBottom:8 }}/>}
          <div style={{ display:"flex", gap:8 }}>
            <Btn onClick={sendTrade} color="#FF9800" style={{ flex:1, padding:"10px 0", fontSize:13 }}>Odoslať 🤝</Btn>
      ...
      <div style={{ padding:"10px 16px", background:"white", borderTop:"1px solid #f0f0f0" }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          ...
          <input
            value={msg}
            onChange={e=>{ setMsg(e.target.value); if(e.target.value.endsWith("@")) setShowMention(true); }}
            onKeyDown={e=>e.key==="Enter"&&sendMsg()}
            placeholder={chatTab==="private" ? "Súkromná správa..." : "Napíš správu..."}
